  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 p-4" onClick={onClose}>
      <div className="w-full max-w-md rounded-xl border border-[hsl(210,60%,50%)]/30 bg-card p-6 shadow-2xl" onClick={(e) => e.stopPropagation()}>
        <h3 className="mb-1 text-center text-lg font-bold text-foreground">Pay with OKX</h3>
        ...
        {loading ? (
          ...
          <>
            <p className="mb-4 text-center text-sm text-muted-foreground">Enter your email to get the payment address</p>
            <input
              type="email"
            ...
            <button onClick={onClose} className="w-full rounded-lg border border-[hsl(210,60%,50%)]/30 py-2 text-sm text-muted-foreground hover:bg-[hsl(210,60%,50%)]/10">Cancel</button>
          </>
          ...
          <div className="text-center">
            ...
            <button onClick={onClose} className="mt-4 w-full rounded-lg border border-[hsl(210,60%,50%)]/30 py-2 text-sm text-muted-foreground hover:bg-[hsl(210,60%,50%)]/10">Close</button>
          </div>
        ) : (
          <>
            ...
            <div className="mb-3 rounded-lg border border-[hsl(210,60%,50%)]/20 bg-background p-4 text-center">
              ...
              <p className="mt-1 text-xs text-muted-foreground">Network: {payInfo.chain}</p>
            </div>
            <div className="mb-3">
              <p className="mb-1 text-xs text-muted-foreground">Send exactly {payInfo.amount} USDT to:</p>
            ...
            <p className="mb-1 text-xs text-muted-foreground">Order ref: <span className="font-mono text-foreground">{payInfo.orderId}</span></p>
        <p className="mb-2 rounded-lg bg-red-500/10 p-3 text-center text-xs text-red-400">
              {locale === "ar" ? "\u064A\u0631\u062C\u0649 \u0625\u0631\u0633\u0627\u0644 \u0627\u0644\u0645\u0628\u0644\u063A + \u0631\u0633\u0648\u0645 \u0627\u0644\u0634\u0628\u0643\u0629 \u0644\u0636\u0645\u0627\u0646 \u0627\u0633\u062A\u0644\u0627\u0645 \u0627\u0644\u062F\u0641\u0639\u0629 \u0628\u0627\u0644\u0643\u0627\u0645\u0644." : "Please send the amount + network fees to ensure the full payment is received."}
            </p>
            <p className="mb-4 rounded-lg bg-yellow-500/10 p-3 text-center text-xs text-yellow-400">
              ⚠️ Send only USDT on {payInfo.chain}. Wrong network = lost funds.
            </p>
            ...
            <Button className="mb-2 w-full bg-green-600 text-white hover:bg-green-700" onClick={handleVerify} disabled={verifying}>
              {verifying ? <><Loader2 className="mr-2 h-4 w-4 animate-spin" /> Checking…</> : "I've Paid — Verify Payment"}
            </Button>
            <button onClick={onClose} className="w-full rounded-lg border border-[hsl(210,60%,50%)]/30 py-2 text-sm text-muted-foreground hover:bg-[hsl(210,60%,50%)]/10">Close</button>
          </>
        )}
      </div>
    </div>
      ...
      {open && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 p-4" onClick={() => setOpen(false)}>
          <div className="w-full max-w-sm rounded-xl border border-[hsl(210,60%,50%)]/30 bg-card p-6 shadow-2xl" onClick={(e) => e.stopPropagation()}>
            <h3 className="mb-1 text-center text-lg font-bold text-foreground">Pay with Crypto</h3>
            <p className="mb-4 text-center text-sm text-muted-foreground">Enter your email to continue</p>
            <input
              type="email"
              placeholder="your@email.com"
              value={email}
            ...
            <button onClick={() => setOpen(false)} className="w-full rounded-lg border border-[hsl(210,60%,50%)]/30 py-2 text-sm text-muted-foreground hover:bg-[hsl(210,60%,50%)]/10">Cancel</button>
          </div>
        </div>
      )}
