...
export function AdminSubscriptions() {
  ...
  const handleEdit = (sub: Sub) => {
    ...
    setForm({
      ...
      plan: sub.plan,
      total_hours: String(sub.total_hours),
      used_hours: String(sub.used_hours),
    ...
    <div className="space-y-4">
          ...
          <Dialog open={dialogOpen} onOpenChange={(open) => { if (!open) resetForm(); setDialogOpen(open) }}>
            ...
            <DialogContent className="max-h-[85vh] overflow-y-auto sm:max-w-lg">
              ...
              <div className="space-y-4 pt-4">
                ...
                <div className="grid grid-cols-2 gap-4">
                  ...
                  {editing && (
                    <div>
                      ...
                      <Input type="number" step="0.5" min="0" value={form.used_hours} onChange={(e) => setForm({ ...form, used_hours: e.target.value })} />
                    </div>
                  )}
                </div>
                <div>
                ...
                <Button onClick={handleSave} disabled={actionLoading === "save" || !form.client_email}>
                  {actionLoading === "save" ? "Saving..." : editing ? "Update" : "Create"}
                </Button>
              </div>
            </DialogContent>
          </Dialog>
      ...
      {loading ? (
        <div className="flex items-center justify-center py-12 text-muted-foreground">
          <RefreshCw className="mr-2 h-5 w-5 animate-spin" /> Loading subscriptions...
        </div>
      ) : subs.length === 0 ? (
        <Card className="border-border bg-card">
          <CardContent className="py-12 text-center text-muted-foreground">No subscriptions yet. Add one to get started.</CardContent>
        </Card>
      ) : (
        <Card className="border-border bg-card">
          <Table>
            ...
            <TableBody>
              {subs.map((sub) => (
                <TableRow key={sub.id}>
                  ...
                  <TableCell>
                    <Badge className={sub.status === "active" ? "bg-emerald-500/20 text-emerald-400" : sub.status === "expired" ? "bg-red-500/20 text-red-400" : "bg-gray-500/20 text-gray-400"}>{sub.status}</Badge>
                  </TableCell>
                  <TableCell className="text-sm">{sub.expires_at ? new Date(sub.expires_at).toLocaleDateString() : "-"}</TableCell>
                  <TableCell>
                    <div className="flex items-center gap-1">
                      <Button size="sm" variant="outline" className="h-7 text-xs" onClick={() => handleEdit(sub)}>
                        <Edit className="mr-1 h-3 w-3" /> Edit
                      </Button>
                      <Button size="sm" variant="outline" className="h-7 text-xs border-red-500/30 text-red-400 hover:bg-red-500/10" disabled={actionLoading === sub.id} onClick={() => handleDelete(sub.id)}>
